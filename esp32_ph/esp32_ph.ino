...
unsigned long timerDelay = 30000; // Mengakses API setiap 30 detik
...
// kontrol autofeeder berdasarkan kondisi pH
float threshold_ph_bawah;
float threshold_ph_atas;
float ph_terkini;

bool alarmAutofeeder = true;  // Status awal alarm autofeeder
bool isOfflineMode = false;  // Tambahkan variabel untuk mengatur mode offline
...
void setup() {
  ...
  wm.setConfigPortalTimeout(300); // Menunggu 5 menit
...
void online() {
  EEPROM.commit();
  //Send an HTTP GET request every 10 minu
  if ((millis() - lastTime) > timerDelay) {
    ...
    if (httpResponseCode > 0) {
      ...
      // Ambil data threshold dan pH terkini
      threshold_ph_bawah = doc["threshold_ph_bawah"];
      threshold_ph_atas = doc["threshold_ph_atas"];
      ph_terkini = doc["ph"];
...
void loop() {
  ...
  if (isOnline && !isOfflineMode) {online();}

  // Logika untuk mengendalikan motor DC berdasarkan kondisi pH
  if (ph_terkini < threshold_ph_bawah || ph_terkini > threshold_ph_atas) {
    // Jika pH berada di luar rentang threshold, pastikan motor mati
    if (alarmAutofeeder) {
      alarmAutofeeder = false;
      digitalWrite(pwindow, LOW);
      digitalWrite(MOTOR, LOW);
    }
  } else {
    // Jika pH dalam rentang aman, motor dapat diaktifkan
    if (!alarmAutofeeder) {
      alarmAutofeeder = true;
    }
  }
  ...
  switch (state) {
    ...
    case menu1:
      ...
      if (cancel) {delay(delayButton);      lcd.clear();      state = awal;}       // kembali ke halaman awal
    ...
    case tanggal:
      ...
      if (cancel) {delay(delayButton);     state = menu1;}
    ...
    case menu2:
      ...
      if (cancel) {delay(delayButton);        lcd.clear();        state = awal;};     // kembali ke menu awal
    ...
    case salarmjam:
      ...
      if (cancel) {delay(delayButton);        state = alarm1;};      // menyimpan jam dan menit alarm
    ...
    case salarmmenit:
      ...
      if (ok)     {delay(delayButton);        i = 0;        state = savealarm;}
      if (cancel) {delay(delayButton);        state = alarm1;}
    ...
    case menu3:
      ...
      if (cancel) {delay(delayButton);        lcd.clear();        state = awal;};       // ke menu awal
    ...
    case menu6:
      ...
      if (ok)    {
        delay(delayButton);
        motordc();
        lcd.clear();  // Tambahkan ini untuk menghapus layar
        state = awal;
      };
      if (cancel) {
        delay(delayButton);
...
void motordc() {
  // Cek status alarmAutofeeder sebelum menjalankan logika motor
  if (!alarmAutofeeder) {
    digitalWrite(MOTOR, LOW);
    digitalWrite(pwindow, LOW);
    return; // Hentikan eksekusi fungsi jika alarm tidak diizinkan

  }
  // Cek status mode offline dan nilai pH sebelum menggerakkan motor
  if (isOfflineMode && (ph_terkini < threshold_ph_bawah || ph_terkini > threshold_ph_atas)) {
    digitalWrite(MOTOR, LOW);
    digitalWrite(pwindow, LOW);
    return;
  }

  for (int angle = 360; angle >= 150; angle -= angleStep) {//pintu menutup
    servo1.write(angle);
    delay(40);
  }
  ...
  // Matikan motor setelah durasi selesai
  analogWrite(MOTOR, 0);
  analogWrite(pwindow, 0);
  delay(2000);

  for (int angle = 150; angle <=   360 ; angle += angleStep) { //pintu membuka, angle awal disesuaikan dengan posisi awal pintu tertutup (dicoba coba secara manual/kira kira dulu)
    servo1.write(angle);
    delay(40);
  }
}
