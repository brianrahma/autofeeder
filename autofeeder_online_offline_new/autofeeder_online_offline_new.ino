...
#include <ESP32Servo.h>//library fix
...
#define EEPROM_SIZE 200 // banyak alamat eeprom yang disediakan
...
int i = 1,
    j, //power
    m,
    m2,//varjam, varmenit, //set alarm jam menit
    h,  //hari
    tl,
    b,
    tn,
...
float tanggalw, bulanw, tahunw, jamw, menitw, takaranw, var0, var1, var2, var3, var4, var5, var6, var7,
      ...
      power1, power2, power3, power4, power5, power6, power7, power8, power9, power10, power11, power12, lv1, lv2, lv3, lv4, lv5;
//koneksi
...
void setup() {
  ...
  isWifiConnected = wm.autoConnect("AFEEDER", "adminkolamkoi"); //AP
  if (!isWifiConnected) {
  ...
  ledcAttachPin(pwindow, ledChannel);
...
void loop() {
  ...
  switch (state) {
    ...
    case awal:
      //Tampilan Awal
      lcd.setCursor(0, 0);
      lcd.print(hari[now.dayOfTheWeek()]);//hari
      lcd.print(" ");
      //lcd.print(now.hour(), DEC); //jam
      lcd.setCursor(7, 0);
      sjam = now.hour();
      if (sjam < 10) {
        lcd.print("0");
      ...
      lcd.print(':');
      //lcd.print(now.minute(), DEC); //menit
      smenit = now.minute();
      lcd.setCursor(10,0);
      if (smenit < 10) {
        lcd.setCursor(10, 0);
      ...
      lcd.setCursor(0, 1);
      lcd.print("   ");
      stanggal = now.day();//tanggal
      if (stanggal < 10) {
        lcd.print("0");
        lcd.print((stanggal), DEC);
      }
      else {
        lcd.print((stanggal), DEC);
      }
    ...
    case tanggal:
      ...
      if (cancel) {
        delay(delayButton);
        //rtc.adjust(DateTime(stanggal, sbulan, stahun));
        rtc.adjust(DateTime((stahun + tn), (b), (tl), (j), (m), 0));
        state = menu1;
    ...
    case salarmjam:
      ...
      if (up)    {
        delay(delayButton);
        varjam[m2]++; if (varjam[m2] > 23)varjam[m2] = 0;
      };
      if (down)  {
        delay(delayButton);
        varjam[m2]--; if (varjam[m2] < 0)varjam[m2] = 23;
      };
      break;
    ...
    case salarmmenit:
      ...
      if (up)    {
        delay(delayButton);
        varmenit[m2]++; if (varmenit[m2] > 59)varmenit[m2] = 0;
      }
      if (down)  {
        delay(delayButton);
        varmenit[m2]--; if (varmenit[m2] < 0)varmenit[m2] = 59;
      }
      break;
    ...
    case takaran1:
      ...
      if (up)    {
        delay(delayButton);
        durasi++;
        if (durasi > 35)durasi = 1;
        dey = (durasi * 1000);
      };
      if (down)  {
        delay(delayButton);
        durasi--;
        if (durasi < 1 )durasi = 35;
        dey = (durasi * 1000);
      };
      break;
...
void motordc() {
  ...
  ledcWrite(ledChannel, 1000);//kekuatan motor ulir saat nyala
  ledcWrite(kuatChannel, kuatlempar);//kekuatan motor pelempar saat nyala

  delay(dey);

  ledcWrite(ledChannel, 0);
  ledcWrite(kuatChannel, 0);
  delay(2000);
...
void online() {
  ...
  if ((millis() - lastTime) > timerDelay) {
    ...
    if (httpResponseCode > 0) {
      ...
      power12 = doc["power12"];
      lv1 = doc["lvw"];
      Serial.println(tanggalw);
      Serial.println(bulanw);
      Serial.println(tahunw);
      Serial.println(jamw);
      Serial.println(menitw);
      Serial.println(takaranw);
      Serial.println(var1);
      Serial.println(var2);
      Serial.println(var3);
      Serial.println(var4);
      ...
      Serial.println(var6);
      Serial.println(var7);
      Serial.println(varjam1);
      Serial.println(varjam2);
      Serial.println(varjam3);
      Serial.println(varjam4);
      Serial.println(varjam5);
      Serial.println(varjam6);
      Serial.println(varjam7);
      Serial.println(varjam8);
      Serial.println(varjam9);
      Serial.println(varjam10);
      Serial.println(varjam11);
      Serial.println(varjam12);
      Serial.println(varmenit1);
      Serial.println(varmenit2);
      Serial.println(varmenit3);
      Serial.println(varmenit4);
      Serial.println(varmenit5);
      Serial.println(varmenit6);
      Serial.println(varmenit7);
      Serial.println(varmenit8);
      Serial.println(varmenit9);
      Serial.println(varmenit10);
      Serial.println(varmenit11);
      Serial.println(varmenit12);
      ...
      Serial.println(power7);
      Serial.println(power8);
      Serial.println(power9);
      Serial.println(power10);
      Serial.println(power11);
      Serial.println(power12);
      ...
      EEPROM.write(eVar[1], var1);
      EEPROM.commit();
      EEPROM.write(eVar[2], var2);
      EEPROM.commit();
      EEPROM.write(eVar[3], var3);
      EEPROM.commit();
      EEPROM.write(eVar[4], var4);
      EEPROM.commit();
      EEPROM.write(eVar[5], var5);
      EEPROM.commit();
      EEPROM.write(eVar[6], var6);
      EEPROM.commit();
      EEPROM.write(eVar[7], var7);
      EEPROM.commit();
      ...
      EEPROM.write(eJam[1], varjam1);
      EEPROM.commit();
      EEPROM.write(eJam[2], varjam2);
      EEPROM.commit();
      EEPROM.write(eJam[3], varjam3);
      EEPROM.commit();
      EEPROM.write(eJam[4], varjam4);
      EEPROM.commit();
      EEPROM.write(eJam[5], varjam5);
      EEPROM.commit();
      EEPROM.write(eJam[6], varjam6);
      EEPROM.commit();
      EEPROM.write(eJam[7], varjam7);
      EEPROM.commit();
      EEPROM.write(eJam[8], varjam8);
      EEPROM.commit();
      EEPROM.write(eJam[9], varjam9);
      EEPROM.commit();
      EEPROM.write(eJam[10], varjam10);
      EEPROM.commit();
      EEPROM.write(eJam[11], varjam11);
      EEPROM.commit();
      EEPROM.write(eJam[12], varjam12);
      EEPROM.commit();
      ...
      EEPROM.write(eMenit[1], varmenit1);
      EEPROM.commit();
      EEPROM.write(eMenit[2], varmenit2);
      EEPROM.commit();
      EEPROM.write(eMenit[3], varmenit3);
      EEPROM.commit();
      EEPROM.write(eMenit[4], varmenit4);
      EEPROM.commit();
      EEPROM.write(eMenit[5], varmenit5);
      EEPROM.commit();
      EEPROM.write(eMenit[6], varmenit6);
      EEPROM.commit();
      EEPROM.write(eMenit[7], varmenit7);
      EEPROM.commit();
      EEPROM.write(eMenit[8], varmenit8);
      EEPROM.commit();
      EEPROM.write(eMenit[9], varmenit9);
      EEPROM.commit();
      EEPROM.write(eMenit[10], varmenit10);
      EEPROM.commit();
      EEPROM.write(eMenit[11], varmenit11);
      EEPROM.commit();
      EEPROM.write(eMenit[12], varmenit12);
      EEPROM.commit();
      ...
      EEPROM.write(eSv[1], power1);
      EEPROM.commit();
      EEPROM.write(eSv[2], power2);
      EEPROM.commit();
      EEPROM.write(eSv[3], power3);
      EEPROM.commit();
      EEPROM.write(eSv[4], power4);
      EEPROM.commit();
      EEPROM.write(eSv[5], power5);
      EEPROM.commit();
      EEPROM.write(eSv[6], power6);
      EEPROM.commit();
      EEPROM.write(eSv[7], power7);
      EEPROM.commit();
      EEPROM.write(eSv[8], power8);
      EEPROM.commit();
      EEPROM.write(eSv[9], power9);
      EEPROM.commit();
      EEPROM.write(eSv[10], power10);
      EEPROM.commit();
      EEPROM.write(eSv[11], power11);
      EEPROM.commit();
      EEPROM.write(eSv[12], power12);
      EEPROM.commit();
...
void  connectingWifi() {
  ...
  isWifiConnected = wm.startConfigPortal("AFEEDER", "adminkolamkoi");
  if (!isWifiConnected) {
